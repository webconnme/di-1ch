...
func SendRequest(b bool) error {

	var messages []webconn.Message
	if b {
		messages = append(messages, webconn.Message{"request", "ok"})
	}
...
func HandleKeyboard() {
	for {
		...
		if ch == 'i' || ch == 'I' {
			SendRequest(true)
			fmt.Println("Sent a digital input on request")
		}
		fmt.Printf("\n[i] digital input : ")
	}
}
...
func main() {
	...
	fmt.Printf("[i] Digital Input : ")
	go HandleNetwork("tcp://192.168.4.180:3005")
	go HandleSignal()
	go HandleKeyboard()
